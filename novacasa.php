                        ...
                        if ($_POST['bairro']<>"") {
                            if ($_POST['dormitorios']<>"") {
                                if ($_POST['suite']<>"") {
                                    if ($_POST['vagas']<>"") {
                                        if ($_POST['banheiros']<>"") {
                                            if ($_POST['aluguel'] == 1 and $_POST['precoal']<>"" or $_POST['aluguel'] == 0 and $_POST['venda'] <> 0) {
                                                if ($_POST['area']<>"") {
                                                    if ($_POST['venda'] == 1 and $_POST['precovenda']<>"" or $_POST['venda'] == 0) {
                            $endereco = mysqli_real_escape_string($conn, $_POST['rua']);
                            $numcasa = mysqli_real_escape_string($conn, $_POST['numcasa']);
                            $endereco .= ', '.$numcasa;
                            $precovenda = mysqli_real_escape_string($conn, $_POST['precovenda']);
                            $precoal = mysqli_real_escape_string($conn, $_POST['precoal']);
                            ...
                            $aluguel = mysqli_real_escape_string($conn, $_POST['aluguel']);    
                                ...
                                $sql = "INSERT INTO casa (endereco, casa, bairro, banheiros, dormitorios, suite, vagas, precovenda, precoal, area, venda, aluguel, cidade, estado) VALUES ('$endereco', '$casaitem', '$bairro', '$banheiros', '$dormitorios', '$suite', '$vagas', '$precovenda', '$precoal', '$area', '$venda' , '$aluguel', '$cidade', '$estado')";
...
?>
    <main>
        ...
        <br>Valor da Venda:<input type="real" name="precovenda"><br>
        <label>Aluguel:</label>
        <select name="aluguel">
        <option value="" selected>Selecione Uma Opcao</option>
        <option value="0">Nao</option>
        ...
        <br>Valor da Locacao:<input type="real" name="precoal"><br>
